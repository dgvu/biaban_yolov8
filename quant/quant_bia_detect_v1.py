...
# Outputs raw tensors: box8, cls8, box16, cls16, box32, cls32.
...
def patch_detect_head(model):
    ...
    def detect_raw_forward(self, x):
        ...
        for i in range(self.nl):
            y = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
            box = y[:, : self.reg_max * 4, :, :]
            cls = y[:, self.reg_max * 4 :, :, :]
            outs.append(box)
            outs.append(cls)
        return tuple(outs)
    ...
    for m in model.modules():
        if m.__class__.__name__ == "Detect":
            ...
            cnt += 1
    print("[PATCH] Detect.forward -> raw box/cls outputs, patched heads:", cnt)
